...
from model import DualPathGenerator, MultiDiscriminator, total_variation_loss
...
def train(args):
    ...
    # ── Phase 1: Pretrain generator (pixel + content only) ──
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for i, (photos, cartoons) in enumerate(loader):
            ...
            with torch.amp.autocast("cuda", enabled=scaler is not None):
                ...
                loss_edge    = edge_fn(fake, cartoons)
            ...
            if i % 10 == 0:
                print(f"Epoch [{epoch}/{args.epochs}] Step [{i}/{len(loader)}] "
                      ...
                      f"edge:{loss_edge.item():.4f} tv:{loss_tv.item():.5f}")
